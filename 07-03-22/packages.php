...
<?php endforeach;  ?>
        ...
        <form role="search" <?php echo $twentytwentyone_aria_label; ?> method="get" class="search-form" action="<?php echo esc_url( home_url( '/' ) ); ?>">
          <div class="search_box">
            <input value="<?php echo get_search_query(); ?>" name="s" type="text" class="form-control" placeholder="Search...">
            <button type="submit" class="btn-search"><i class="zmdi zmdi-search"></i></button>
          </div>
        </form>
          <form method="get" action="">
          <div class="filter">
...
?>
              <li>
                <div class="form-group">
                  <input type="checkbox" <?php if (in_array($wcatTerm->slug, $dd)){?>checked<?php } ?> class="d" name="d" value="<?php echo $wcatTerm->slug; ?>" id="checkdd-<?php echo $wcatTerm->term_id; ?>">
...
//d for = destination
$wcatTerms = get_terms(
'pprice', array('hide_empty' => 0,  'order' =>'asc', 'parent' =>0));
        foreach($wcatTerms as $wcatTerm) : 
?>
<li>
  <div class="form-group">
    <input type="checkbox" <?php if (in_array($wcatTerm->slug, $ppp)){?>checked<?php } ?> class="dp" name="dp" value="<?php echo $wcatTerm->slug; ?>" id="checkpp-<?php echo $wcatTerm->term_id; ?>">
    <label for="checkpp-<?php echo $wcatTerm->term_id; ?>"><?php echo $wcatTerm->name; ?></label>
  </div>
</li>
<?php endforeach;  ?>            
...
//d for = destination
$wcatTerms = get_terms(
'sstar', array('hide_empty' => 0,  'order' =>'asc', 'parent' =>0));
        foreach($wcatTerms as $wcatTerm) : 
?>
<li>
  <div class="form-group">
    <input type="checkbox" <?php if (in_array($wcatTerm->slug, $ss)){?>checked<?php } ?> class="ss" name="ss" value="<?php echo $wcatTerm->slug; ?>" id="checks-<?php echo $wcatTerm->term_id; ?>">
    <label for="checks-<?php echo $wcatTerm->term_id; ?>"><?php echo $wcatTerm->name; ?> Star</label>
  </div>
</li>
<?php endforeach;  ?>  
...
//d for = destination
$wcatTerms = get_terms(
'inclusions', array('hide_empty' => 0,  'order' =>'asc', 'parent' =>0));
        foreach($wcatTerms as $wcatTerm) : 
          $catImage = get_field('inclusions_image',$wcatTerm)
?>
<li>
  <div class="form-group">
    <input type="checkbox" <?php if (in_array($wcatTerm->slug, $inn)){?>checked<?php } ?> class="in" name="in" value="<?php echo $wcatTerm->slug; ?>" id="checkin-<?php echo $wcatTerm->term_id; ?>">
    <label for="checkin-<?php echo $wcatTerm->term_id; ?>"><img src="<?php echo $catImage['url'];?>" alt="<?php echo $catImage['alt'];?>" />hghg</label>
  </div>
</li>
<?php endforeach;  ?>  
...
//echo $p;
 ...
 $todate = $_GET['todate'];
  ...
  if($dedate){
    //echo $dedate ;
    $meta_query[] = ['key'=> 'travel_form','value'=>$dedate,'type' => 'DATE',];
  }  
  if($todate){
    //echo $todate ;
    $meta_query[] = ['key'=> 'to_date','value'=>$todate,'type' => 'DATE',];
  } 
//Home Page search
if (!empty($depart)) 
{
  $tax_query[]=['taxonomy' => 'depart_form','field' => 'id','terms' =>$depart];
}
if (!empty($going)) 
{
  $tax_query[]=['taxonomy' => 'going_to','field' => 'id','terms' =>$going];
}
if (!empty($persons)) 
{
  $tax_query[]=['taxonomy' => 'persons','field' => 'id','terms' =>$persons];
}
...
// $property_packages = array_filter($packages);
// var_dump($property_packages);
if (!empty($p)) 
{
 //var_dump(explode(',',$pp));
  $tax_query[]=['taxonomy' => 'packages', 'field' => 'slug','terms' =>explode(',',$p)];
  //var_dump($tax_query);
}
if (!empty($d)) 
{
  $tax_query[]=['taxonomy' => 'destination','field' => 'slug','terms' =>explode(',',$d)];
}
if (!empty($pp)) 
{
  $tax_query[]=['taxonomy' => 'pprice','field' => 'slug','terms' =>explode(',',$pp)];
}
if (!empty($s)) 
{
  $tax_query[]=['taxonomy' => 'sstar','field' => 'slug','terms' =>explode(',',$s)];
}
if (!empty($in)) 
{
  $tax_query[]=['taxonomy' => 'inclusions','field' => 'slug','terms' =>explode(',',$in)];
}
